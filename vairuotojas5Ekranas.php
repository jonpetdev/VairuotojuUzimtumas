...
<?php
	...
	$result = $conn->query( "SELECT * FROM vairuotojai_uzim WHERE id = '5'") or die(mysql_error());
		...
		while($row=mysqli_fetch_array($result)){
			...
			if($row['darbas']=='dirba'){
				...
				if($row['aktyvus']=='vair'){
					...
					echo "<div ><iframe class='uzsakymas1' src='frame1.php' onload='resizeIframe(this)' scrolling='no' frameborder='no'  > </iframe></div>";
					echo "<div ><iframe class='uzsakymas2' src='frame1_2.php' onload='resizeIframe(this)' scrolling='no' frameborder='no'  > </iframe></div>";
